...
int yeni_sehir_ekle()
{
    ...
    if(yeni_plaka==1)
    {
        printf("\nPlaka kodu 1 olarak girildigi için sehir listenin basina eklenmistir..");
        secim=1;
        goto label6;
    }
    if(yeni_plaka>son->plaka_kodu+2)
    {
        printf("\nPlaka kodu listenin sonundaki sehirin plaka kodundan buyuk oldugu icin girilen plaka kodu degeri mevcut listedeki en buyuk plaka kodu degerinin bir fazlasi olarak guncellenmistir(-%d-)..",son->plaka_kodu+1);
        yeni_plaka=(son->plaka_kodu)+1;
        secim=2;
        goto label6;
    }
    if(yeni_plaka>son->plaka_kodu+1)
    {
        printf("\nPlaka kodu listenin sonundaki sehirin plaka kodundan 1 fazla oldugu icin yeni sehir,%d. sehir olarak listeye eklenmistir..",son->plaka_kodu+1);
        secim=2;
         goto label6;  //sonra ki if yapisini atlamak icin label kullandik..
    }

    if(yeni_plaka==son->plaka_kodu)
    {
        printf("\nPlaka kodu listenin sonundaki sehirin plaka koduna esit oldugu icin yeni sehir , %d. sehir olarak listeye yerlestirilmistir..",son->plaka_kodu);
        secim=3;
        goto label6;
    }

    if(yeni_plaka>1&&yeni_plaka<son->plaka_kodu)
    {
        printf("\nPlaka kodu 1 ile %d arasinda oldugu icin yeni sehir ,  plaka sirasi bozulmayacak sekilde listeye yerlestirilmitir..",son->plaka_kodu);
        secim=3;
    }
     label6:
    switch(secim)
